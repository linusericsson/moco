...
parser.add_argument('--sub-batches', default=4, type=int,
                    help='number of sub batches to perform (default: 4)')
...
parser.add_argument('--augmentations', nargs='+', help='<Required> Set flag', required=True)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    augs_dict = {
        ...
        #'translation': transforms.RandomAffine(0, translate=((crop_size / resize) / 4, (crop_size / resize) / 4)),
        #'rotation': transforms.RandomAffine(180),
        'resized_crop': transforms.RandomResizedCrop(224, scale=(0.2, 1.)),
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    b = int(args.batch_size / args.sub_batches)
    assert args.batch_size / args.sub_batches == b, 'The batch-size must be divisible by the number of sub-batches'
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        # this is where we do things three times
        for j in range(args.sub_batches):
            if args.gpu is not None:
                im_q = images[0][j * b: (j + 1) * b].cuda(args.gpu, non_blocking=True)
                im_k = images[1][j * b: (j + 1) * b].cuda(args.gpu, non_blocking=True)
